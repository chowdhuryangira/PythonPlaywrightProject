...
@pytest.mark.parametrize("email,password,username", [("mail2sapachowdhury@gmail.com", "abc123", "user1")])
def test_login_user(set_up_ecommerce_website, email, password, username):
    ...
    assert page.get_by_text(" Logged in as ").is_visible(timeout=3000), "Login failed or username not displayed"
    assert page.locator('//a[contains(text(), "Logged in as")]/b').text_content() == username,  "Username does not match"
    ...
    assert page.get_by_text("Login to your account").is_visible(timeout=3000), "Logout failed or login page not displayed"
